...
function ColectasInner({ soloArribos = false, irA }) {
  ...
  const estEf = (c) => estadoEfectivo(c, registros[c.id]);
  ...
  const zoneTabs = (
    <div style={{ display:'flex', gap:0, marginBottom:20, borderBottom:`1px solid ${BRAND.border}` }}>
      {SECCIONES.map(s => {
        const active = tab === s;
        // El badge "sin confirmar" solo tiene sentido si esa colecta ocurre el día que estás mirando:
        //  · entre semana, la colecta de SÁBADOS todavía no pasó → no alerta.
        //  · en sábado, los clientes de CABA/SUR/NOROESTE no operan → tampoco alertan (si no, un
          ...
          {vistaColectas === 'mapa' ? (
            ...
            <Suspense fallback={<div style={{ color:BRAND.muted, padding:'3rem', textAlign:'center' }}>Cargando mapa…</div>}>
              <ColectasMapa
                ...
                registros={registros}
                choferesFull={choferesFull}
                updateRegistro={updateRegistro}
                onGeoUpdate={guardarGeoCliente}
                fecha={fecha}
                setFecha={setFecha}
                tab={tab}
                esMovil={esMovil}
