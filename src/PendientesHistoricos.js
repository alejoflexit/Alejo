...
export default function PendientesHistoricos() {
  ...
  const [service, setService] = useState("Todos"), [state, setState] = useState("Todos"), [query, setQuery] = useState(""), [selected, setSelected] = useState(null), [tipDay, setTipDay] = useState(null);
  ...
  return <div style={{ color:"#fff" }}>
    ...
    <div style={calendar}><div style={{ width:"100%", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, marginBottom:10 }}><div><b style={{ fontSize:15 }}>Pendientes por día</b><small style={muted}>Fecha de origen · ingreso A planta</small></div><div style={{ display:"flex", alignItems:"center", gap:6 }}><span style={muted}>Semana seleccionada</span><button style={button}>←</button><button style={button}>Semana actual</button><button style={button}>→</button></div></div><div style={daysGrid}>{calendarDays.map(({key,d,rs}) => <div key={key} style={{ position:"relative" }}><button onClick={() => { setDay(key); setTipDay(null); }} onMouseEnter={() => setTipDay(key)} onMouseLeave={() => setTipDay(null)} style={{ ...dayCard, ...(day===key?dayActive:{}) }}><small style={{ textTransform:"capitalize" }}>{d.toLocaleDateString("es-AR", { weekday:"long" })}</small><b style={{ fontSize:14 }}>{d.getDate()}</b><strong>{rs.length || "—"}</strong><small>{rs.length ? "pendientes" : "sin cobertura"}</small></button>{tipDay===key && <div style={tooltip}><b>{labelDate(key)}</b><div>Flex: {rs.filter(r=>r.service==="Flex").length}</div><div>Particulares: {rs.filter(r=>r.service==="Particular").length}</div></div>}</div>)}</div></div>
    <div style={kpiGrid}><div style={kpi}>Flex pendientes <b>{count("Flex")}</b></div><div style={kpi}>Particulares <b>{count("Particular")}</b></div><div style={kpi}>Total pendientes <b>{count("Todos")}</b></div><div style={{ ...kpi, color:"#ff8f9a" }}>Prioridad crítica <b>{rows.filter(r=>/nadie|no entregado/i.test(r.estado||"")).length}</b></div></div>
    <div style={serviceBar}><div style={segmented}>{["Todos","Flex","Particular"].map(s => <button key={s} onClick={() => setService(s)} style={{ ...button, ...(service===s?active:{}) }}>{s} <b>{count(s)}</b></button>)}</div><label style={{ ...label, flexDirection:"row", alignItems:"center" }}>Estado<select value={state} onChange={e => setState(e.target.value)} style={input}><option>Todos</option>{states.map(s=><option key={s}>{s}</option>)}</select></label></div>
