...
export default function PendientesHistoricos() {
  ...
  const copiar = async row => {
    ...
    } catch { setCopiado("error:" + row.id_interno); setTimeout(() => setCopiado(""), 2500); }
  };
  ...
  return <div style={{ color:"#fff" }}>
    ...
    <div style={{ ...card, borderTop:0, borderRadius:"0 0 10px 10px", overflowX:"auto" }}><table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}><thead><tr>{["Fecha de origen","Servicio / envío","Asignado a","Cliente / dirección","Estado","Mensaje"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead><tbody>{loading && rows.length === 0 ? <tr><td colSpan="6" style={empty}>Cargando pendientes…</td></tr> : rows.length===0 ? <tr><td colSpan="6" style={empty}><b>No hay datos históricos cargados.</b><br/><small>La consulta respondió correctamente, pero la caché de envíos está vacía. Hay que ejecutar la sincronización de LightData.</small></td></tr> : visible.length===0 ? <tr><td colSpan="6" style={empty}>No hay pendientes para estos filtros.</td></tr> : visible.map(r => <tr key={r.id_interno} onClick={()=>setSelected(r)} style={{ borderTop:"1px solid rgba(255,255,255,.08)", cursor:"pointer" }}><td style={td}><b>{labelDate(r.origin)}</b></td><td style={td}><span style={{ ...pill, ...(r.service==="Flex"?flexPill:{}) }}>{r.service}</span><small style={muted}>{r.id_venta_ml || r.tracking || r.id_interno}</small></td><td style={td}><b>{r.cadete || "Sin asignar"}</b><small style={muted}>Último movimiento: {labelDate(r.fecha_estado)}</small></td><td style={td}><b>{r.razon_social || "Cliente sin nombre"}</b><small style={muted}>{[r.direccion,r.localidad].filter(Boolean).join(" · ") || "Dirección no informada"}</small></td><td style={td}>{r.estado || "Sin estado"}<small style={{...muted,color:pendingPriority(r).color,fontWeight:700}}>● {pendingPriority(r).label}</small></td><td style={td}><button onClick={e => { e.stopPropagation(); copiar(r); }} style={{ ...copyButton, ...(copiado === r.id_interno ? copyButtonOk : {}) }}>{copiado === r.id_interno ? "Copiado ✓" : copiado === "error:" + r.id_interno ? "No se pudo" : "Copiar mensaje"}</button></td></tr>)}</tbody></table></div>
    {selected && <div role="dialog" onClick={()=>setSelected(null)} style={overlay}><div onClick={e=>e.stopPropagation()} style={drawer}><button onClick={()=>setSelected(null)} style={{ ...button, float:"right" }}>×</button><div style={muted}>DETALLE DEL ENVÍO</div><h2>{selected.id_venta_ml || selected.tracking || selected.id_interno}</h2><p><b>{selected.service}</b> · {selected.estado || "Sin estado"}</p><hr/><p><b>Fecha de origen</b><br/>{labelDate(selected.origin)}</p><p><b>Asignado a</b><br/>{selected.cadete || "Sin asignar"}</p><p><b>Cliente</b><br/>{selected.razon_social || "Sin nombre"}</p><p><b>Dirección</b><br/>{[selected.direccion,selected.localidad].filter(Boolean).join(" · ") || "No informada"}</p><hr/><div style={muted}>MENSAJE PARA EL CADETE</div><pre style={mensajePreview}>{mensajeCadete(selected)}</pre><button onClick={() => copiar(selected)} style={{ ...copyButton, ...(copiado === selected.id_interno ? copyButtonOk : {}) }}>{copiado === selected.id_interno ? "Copiado ✓" : copiado === "error:" + selected.id_interno ? "No se pudo copiar" : "Copiar mensaje"}</button></div></div>}
  </div>;
}
