...
export default function PendientesHistoricos() {
  ...
  const [service, setService] = useState("Todos"), [state, setState] = useState([...OPEN_STATES]), [query, setQuery] = useState(""), [selected, setSelected] = useState(null), [tipDay, setTipDay] = useState(null);
  const load = async () => {
  ...
  const calendarDays = useMemo(() => {
    ...
    return Array.from({ length: 7 }, (_, i) => { const d = new Date(start); d.setDate(start.getDate() + i); const key = d.toISOString().slice(0, 10); const rs = rows.filter(r => r.origin === key && isOpenShipment(r)); return { key, d, rs }; });
  }, [rows]);
