...
import { pendingPriority } from "./pendingPriority";
...
export default function PendientesHistoricos() {
  ...
  const primaryStates = ["En camino al destinatario", "En planta de procesamiento", "Nadie", "Nadie 2DA visita", "No entregado"];
  const visibleStates = stateExpanded ? states : states.filter(s => primaryStates.includes(s));
  const visible = useMemo(() => rows.filter(r => (!day || r.origin === day) && (!criticalOnly || pendingPriority(r).rank === 3) && (service === "Todos" || r.service === service) && (state.length === 0 || state.includes(String(r.estado || "Sin estado").trim())) && `${r.id_venta_ml} ${r.tracking} ${r.razon_social} ${r.cadete} ${r.direccion} ${r.localidad}`.toLowerCase().includes(query.toLowerCase())).sort((a,b) => pendingPriority(b).rank - pendingPriority(a).rank || String(a.origin).localeCompare(String(b.origin)) || Number(b.service === "Flex") - Number(a.service === "Flex")), [rows,day,service,state,query,criticalOnly]);
  ...
  return <div style={{ color:"#fff" }}>
    ...
    <div style={urgentHero}><div><strong style={{ fontSize:34, lineHeight:1 }}>{rows.filter(r => (!day || r.origin === day) && r.service === "Flex" && pendingPriority(r).rank === 3).length}</strong><div><b>Flex con más de 48 horas</b><small>Estos envíos necesitan seguimiento prioritario.</small></div></div><button onClick={() => { setCriticalOnly(true); setDay(""); }} style={urgentButton}>Revisar urgentes →</button></div>
    ...
    <div style={kpiGrid}><div style={kpi}>Flex pendientes <b>{count("Flex")}</b></div><div style={kpi}>Particulares <b>{count("Particular")}</b></div><div style={kpi}>Total pendientes <b>{count("Todos")}</b></div><div style={{ ...kpi, color:"#ff8f9a" }}>Prioridad crítica <b>{rows.filter(r => (!day || r.origin === day) && pendingPriority(r).rank === 3).length}</b></div></div>
    <div style={serviceBar}><div style={segmented}>{["Todos","Flex","Particular"].map(s => <button key={s} onClick={() => setService(s)} style={{ ...button, ...(service===s?active:{}) }}>{s} <b>{count(s)}</b></button>)}</div><div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}><span style={{ ...muted, marginTop:0 }}>Estado</span><button aria-pressed={state.length===0} onClick={() => setState([])} style={{ ...button, ...(state.length===0?active:{}) }}>Todos</button>{visibleStates.map(s=><button key={s} aria-pressed={state.includes(s)} onClick={() => setState(prev => prev.includes(s) ? prev.filter(x=>x!==s) : [...prev,s])} style={{ ...button, ...(state.includes(s)?active:{}) }}>{s}</button>)}<button onClick={() => setStateExpanded(v => !v)} style={{ ...button, ...(stateExpanded?active:{}) }} aria-expanded={stateExpanded}>{stateExpanded ? "Menos" : "···"}</button></div></div>
    <div style={{display:"flex",gap:8,margin:"10px 0",flexWrap:"wrap"}}><button style={{...button,...(criticalOnly?active:{})}} aria-pressed={criticalOnly} onClick={()=>{setCriticalOnly(v=>!v);setDay("");}}>Flex +48 h · todos los días</button><button style={button} onClick={()=>{setDay(argentinaYesterday());setCriticalOnly(false);}}>Ayer</button><button style={button} onClick={()=>setDay("")}>Ver todos los días</button><span style={muted}>{day ? labelDate(day) : "Todo el historial"} · {visible.length} pendientes</span></div><div style={filters}><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar envío, cliente, cadete o dirección…" style={{ ...input, flex:1, minWidth:220 }}/></div>
