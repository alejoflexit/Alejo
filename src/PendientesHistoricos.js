...
export default function PendientesHistoricos() {
  ...
  return <div style={{ color:"#fff" }}>
    ...
    <div style={calendar}><div style={{ width:"100%", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, marginBottom:10 }}><div><b style={{ fontSize:15 }}>Pendientes por día</b><small style={muted}>Fecha de origen · ingreso A planta</small></div><button onClick={() => setDay("")} style={linkButton}>Ver todos los días →</button></div><div style={daysGrid}>{calendarDays.map(({key,d,rs}) => <div key={key} style={{ position:"relative" }}><button onClick={() => { setDay(key); setTipDay(null); }} onMouseEnter={() => setTipDay(key)} onMouseLeave={() => setTipDay(null)} style={{ ...dayCard, ...(day===key?dayActive:{}) }}><small style={{ textTransform:"capitalize" }}>{d.toLocaleDateString("es-AR", { weekday:"long" })}</small><b style={{ fontSize:14 }}>{d.getDate()}</b><strong>{rs.length || "—"}</strong><small>{rs.length ? "pendientes" : "sin datos"}</small></button>{tipDay===key && <div style={tooltip}><b>{labelDate(key)}</b><div>Flex: {rs.filter(r=>r.service==="Flex").length}</div><div>Particulares: {rs.filter(r=>r.service==="Particular").length}</div></div>}</div>)}</div></div>
