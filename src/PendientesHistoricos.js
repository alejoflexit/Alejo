...
export default function PendientesHistoricos() {
  ...
  const load = async () => { setLoading(true); setError(""); try {
    ...
    const data = await res.json(); setRows(data.filter(r => !resolved.test(String(r.estado || ""))).map(r => ({ ...r, service:serviceOf(r), origin:isoDate(r.fecha_flexit) || isoDate(r.fecha_estado) })));
  ...
  return <div style={{ color:"#fff" }}>
    ...
    <div style={calendar}><label style={label}>Fecha de origen<input type="date" value={day} onChange={e => setDay(e.target.value)} style={input}/></label><div style={segmented}>{["Todos","Flex","Particular"].map(s => <button key={s} onClick={() => setService(s)} style={{ ...button, ...(service===s?active:{}) }}>{s} <b>{count(s)}</b></button>)}</div></div>
