...
export default function PendientesHistoricos() {
  ...
  const load = async () => { setLoading(true); setError(""); try {
    ...
    if (available.includes(argentinaYesterday())) setDay(argentinaYesterday());
  } catch (e) { setError(e.message); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
  const states = useMemo(() => [...new Set(rows.map(r => String(r.estado || "Sin estado").trim()))].sort(), [rows]);
  const visible = useMemo(() => rows.filter(r => (!day || r.origin === day) && (service === "Todos" || r.service === service) && (state.length === 0 || state.includes(String(r.estado || "Sin estado").trim())) && `${r.id_venta_ml} ${r.tracking} ${r.razon_social} ${r.cadete} ${r.direccion} ${r.localidad}`.toLowerCase().includes(query.toLowerCase())).sort((a,b) => Number(/nadie|no entregado/i.test(b.estado||"")) - Number(/nadie|no entregado/i.test(a.estado||"")) || String(a.origin).localeCompare(String(b.origin)) || Number(b.service === "Flex") - Number(a.service === "Flex")), [rows,day,service,state,query]);
  ...
  return <div style={{ color:"#fff" }}>
    ...
    <div style={{ display:"flex", gap:6, flexWrap:"wrap", margin:"8px 0" }}>{day && <span style={chip}>{labelDate(day)} ×</span>}{service!=="Todos" && <span style={chip}>{service} ×</span>}{state.map(s=><span key={s} style={chip}>{s} ×</span>)}</div><div style={filters}><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar envío, cliente, cadete o dirección…" style={{ ...input, flex:1, minWidth:220 }}/></div>
