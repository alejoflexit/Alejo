...
export default function Seguimiento({ semanas, semanaActiva, session }) {
  ...
  const [enviados, setEnviados] = useState([]), [inactivos, setInactivos] = useState([]), [verInactivos, setVerInactivos] = useState(false), [configAbierta, setConfigAbierta] = useState(false), [abierto, setAbierto] = useState(null), [copiado, setCopiado] = useState(null), [busy, setBusy] = useState(null), [error, setError] = useState("");
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{padding:"1px 0 3px",display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}><div><div style={{fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif",fontSize:18,fontWeight:650,letterSpacing:"-.15px",lineHeight:1.25,color:C.white}}>Seguimiento semanal de cadetes</div><div style={{color:"rgba(255,255,255,.68)",fontSize:12.5,fontWeight:400,lineHeight:1.4,marginTop:5}}>SLA Meli de {semanaActiva}{reporte.anterior?` · comparado con ${reporte.anterior.label}`:" · sin semana anterior"}</div></div>{esAdmin&&<button aria-label="Configurar cadetes" aria-expanded={configAbierta} title="Configurar cadetes" onClick={()=>setConfigAbierta(x=>!x)} style={botonIcono(configAbierta)}><GearIcon/></button>}</div>
    ...
    {esAdmin&&configAbierta&&<div style={{...card,padding:"14px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:10}}><div><b style={{color:C.white,fontSize:14}}>Configurar cadetes</b><div style={{color:C.muted,fontSize:11.5,marginTop:3}}>Ocultalos del seguimiento sin borrar sus métricas.</div></div><button onClick={()=>setConfigAbierta(false)} aria-label="Cerrar configuración" style={{border:0,background:"transparent",color:C.muted,cursor:"pointer",fontSize:20,lineHeight:1}}>×</button></div><div style={{display:"grid",gap:7}}>{reporte.filas.map(f=><div key={f.cadete} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,padding:"8px 10px",borderRadius:8,background:"rgba(255,255,255,.035)"}}><span style={{color:C.white,fontSize:12.5,fontWeight:650}}>{f.cadete}</span><button disabled={busy===`baja:${f.cadete}`} onClick={()=>baja(f)} style={boton(C.red)}>{busy===`baja:${f.cadete}`?"Ocultando…":"Ocultar"}</button></div>)}</div></div>}
    ...
    {reporte.filas.length===0?<div style={{...card,textAlign:"center",padding:"2.5rem",color:C.teal}}><i className="ti ti-circle-check" style={{fontSize:34,display:"block",marginBottom:8}}/>Todos los cadetes quedaron en 98% o más.</div>:reporte.filas.map(f=>{
      ...
      return <div key={f.cadete} style={{...card,borderColor:enviado?"rgba(46,207,170,.38)":f.critico?"rgba(226,75,74,.34)":C.border}}>
        <div style={{display:"flex",gap:12,alignItems:"flex-start",flexWrap:"wrap"}}><div style={{flex:"1 1 240px"}}>
          <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}><b style={{color:C.white,fontSize:15}}>{f.cadete}</b>{f.critico&&<small style={estadoChip(C.red)}>Crítico</small>}{f.reincidente&&<small style={estadoChip(C.amber)}>Reincidente</small>}{f.muestraChica&&<small style={estadoChip(C.amber)}>Muestra chica</small>}{enviado&&<small style={{color:C.teal,fontWeight:800}}>✓ ENVIADO {fechaEnvio(enviado.enviado_at)}</small>}</div>
...
function estadoChip(color){const rojo=color===C.red;return{color,background:rojo?"rgba(226,75,74,.11)":"rgba(239,159,39,.1)",border:`1px solid ${rojo?"rgba(226,75,74,.24)":"rgba(239,159,39,.23)"}`,borderRadius:999,padding:"3px 8px",fontSize:11,fontWeight:700,lineHeight:1.2};}
function GearIcon(){return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.88l.06.06-2.83 2.83-.06-.06a1.7 1.7 0 0 0-1.88-.34 1.7 1.7 0 0 0-1.03 1.56V21h-4v-.08A1.7 1.7 0 0 0 8.94 19.4a1.7 1.7 0 0 0-1.88.34l-.06.06-2.83-2.83.06-.06A1.7 1.7 0 0 0 4.57 15 1.7 1.7 0 0 0 3 14H3v-4h.08A1.7 1.7 0 0 0 4.6 8.94a1.7 1.7 0 0 0-.34-1.88L4.2 7l2.83-2.83.06.06a1.7 1.7 0 0 0 1.88.34A1.7 1.7 0 0 0 10 3.08V3h4v.08a1.7 1.7 0 0 0 1.03 1.49 1.7 1.7 0 0 0 1.88-.34l.06-.06L19.8 7l-.06.06a1.7 1.7 0 0 0-.34 1.88A1.7 1.7 0 0 0 20.92 10H21v4h-.08A1.7 1.7 0 0 0 19.4 15z"/></svg>}
