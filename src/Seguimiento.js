...
export default function Seguimiento({ semanas, semanaActiva, session }) {
  ...
  const [enviados, setEnviados] = useState([]), [inactivos, setInactivos] = useState([]), [verInactivos, setVerInactivos] = useState(false), [abierto, setAbierto] = useState(null), [copiado, setCopiado] = useState(null), [busy, setBusy] = useState(null), [error, setError] = useState("");
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:10}}>
      {[["Para contactar",accionables,C.red],["Críticos",criticos,C.red],["Reincidentes",reincidentes,C.amber],["Mensajes enviados",enviados.length,C.teal]].map(([l,v,c])=><div key={l} style={card}><div style={{color:C.muted,fontSize:11,textTransform:"uppercase"}}>{l}</div><div style={{color:c,fontSize:26,fontWeight:800,marginTop:5}}>{v}</div></div>)}
    </div>
