...
export default function Seguimiento({ semanas, semanaActiva, session }) {
  ...
  const baja = async f => {
    if(!esAdmin) return;
    if(!window.confirm(`¿Dar de baja a ${f.cadete}?\n\nDejará de aparecer en Seguimiento, pero conservaremos sus métricas históricas.`)) return;
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {reporte.filas.length===0?<div style={{...card,textAlign:"center",padding:"2.5rem",color:C.teal}}><i className="ti ti-circle-check" style={{fontSize:34,display:"block",marginBottom:8}}/>Todos los cadetes quedaron en 98% o más.</div>:reporte.filas.map((f,i)=>{
      ...
      return <div key={f.cadete} style={{...card,borderColor:enviado?"rgba(46,207,170,.38)":f.critico?"rgba(226,75,74,.34)":C.border}}>
        <div style={{display:"flex",gap:12,alignItems:"flex-start",flexWrap:"wrap"}}><div style={{width:28,height:28,borderRadius:8,background:"rgba(255,255,255,.06)",color:C.muted,display:"grid",placeItems:"center",fontSize:12,fontWeight:700,fontVariantNumeric:"tabular-nums"}}>{i+1}</div><div style={{flex:"1 1 240px"}}>
        ...
        </div><div style={{display:"flex",alignItems:"flex-start",gap:8}}>{esAdmin&&<button aria-label={`Configurar ${f.cadete}`} title="Configurar cadete" onClick={()=>setConfigCadete(configCadete===f.cadete?null:f.cadete)} style={botonIcono(configCadete===f.cadete)}><i className="ti ti-settings"/></button>}<div style={{textAlign:"right"}}><div style={{color,fontFamily:'"Space Grotesk",sans-serif',fontSize:22,fontWeight:700,letterSpacing:"-.4px",fontVariantNumeric:"tabular-nums"}}>{fmt(f.sla)}%</div><div style={{color:C.muted,fontSize:10,fontVariantNumeric:"tabular-nums"}}>{f.slaAnterior===null?"sin dato anterior":`antes ${fmt(f.slaAnterior)}%`}</div></div></div></div>
        ...
        {configCadete===f.cadete&&<div style={{marginTop:10,padding:"10px 12px",borderRadius:9,background:"rgba(255,255,255,.035)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,color:C.muted,fontSize:12}}><span>Administrar este cadete</span><button disabled={busy===`baja:${f.cadete}`} onClick={()=>baja(f)} style={boton(C.red)}>{busy===`baja:${f.cadete}`?"Dando de baja…":"Dar de baja del seguimiento"}</button></div>}
